...
# -------- PDF 관련 --------
from ..schemas import DetectResponse, PatternItem, Box, XmlScanResponse
...
async def pdf_scan_endpoint(
    ...
    try:
        ...
        for p in _parse_patterns_json(patterns_json):
            rx = _compile(p)
...
async def xml_apply_endpoint(
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if lower.endswith(".docx"):
            ...
            out_path = str(Path(td) / f"{stem}.redacted.hwpx")  # ← 여분의 '}' 제거
